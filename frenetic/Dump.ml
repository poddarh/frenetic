...
module Local = struct
  ...
  let run file nr_switches printfdd dumpfdd json () =
...
module Global = struct
  ...
  let run file printfdd dumpfdd printauto dumpauto new_comp json () =
    let pol = parse_pol ~json file in
    let (t, fdd) =
      if new_comp then
        time (fun () -> Frenetic_NetKAT_Global_Compiler.compile pol)
      else
         time (fun () -> Frenetic_NetKAT_Compiler.compile_global pol)
    in
